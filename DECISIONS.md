...
## Q62 — herdr-advisor/never-ask-first — gate-resolution

**Question:** The Antigravity advisor of Q61 died on a permission dialog, and the HARNESS-CLIS.md intro listed "never ask" and "never write" among eight capabilities with no priority between them: three rows (Antigravity, Cursor, Devin) had kept a read-only shape that prompts, while five others had already taken never-ask over never-write. Which comes first, and what does a row take when its CLI cannot deliver both the way Claude Code and Codex do?
**Options considered:** approve-all outright for any CLI without a deny-and-carry-on read-only mode / never-ask first, then never-write: the narrowest configuration that never prompts, keeping a silent refusal of writes where the CLI has one and taking approve-all only where it has none / leave the three rows and keep pointing at the user's own settings for a deny; and for trust dialogs: in scope as an ask like any other / out of scope, since the advisor launches in the worker's already-trusted cwd
**Chosen:** Never-ask first, then never-write, trust dialogs included, stated once in the HARNESS-CLIS.md intro; SKILL.md's "deny-and-carry-on" line now reads "a mode that never prompts, refusing or approving"; the wrappers paragraph keeps its stripping rule, reworded to "drop the wrapper's own permission flags, since the row supplies the mode". Antigravity takes `--dangerously-skip-permissions`; Cursor takes `-f --trust` on top of `--mode=ask`; Devin takes `--permission-mode bypass --respect-workspace-trust false`, and `devin.json` loses its permission rules. `launch-test.sh`'s prompt is a four-step probe (an out-of-workspace read, `herdr agent list`, `date`, a write), with a one-time resend for a probe that never landed, the reply read from recent output, a captured dialog on `blocked`, a per-run probe file name and a `wrote=yes|no` field.
**Decided-by:** user
**Justification:** Grilling session, 2026-09-18, all recommendations accepted. The three rows were probed live from a scratch directory. Antigravity, with `allowNonWorkspaceAccess` flipped off for the run and restored after: outside read, both commands and the write ran unprompted in an untrusted directory, no diff review held the write, and its log confirms `--dangerously-skip-permissions: auto-approving all tool permissions`; one run's `wrote=no` was a concurrent Cursor run's cleanup in the shared directory, which is why the probe file is now per run, and one run's prompt never landed, which is why the probe is resent once. Cursor: the free plan refuses every named model, the config's default included, so the launch note says `auto`; under `--mode=ask -f --trust` the outside read and both commands ran and the agent refused the write itself ("Ask mode blocks file edits"), so that row now asks for nothing and stays read-only, with only its open shell as a Gap. Devin: under its config deny the write stopped `blocked` at `Writing ./advisor-probe.txt` with `1 Yes (Approve once) … 4 No`, twice; under bypass the probe wrote the file, so a local deny survives neither, and the config keeps only the org id and the finished shell setup. Approve-all outright was rejected because Cursor gets read-only for free; leaving the rows was rejected because the Gaps list exists so the worker chooses knowing, and a row that hangs on the manual's first step is not a choice.
**Outcome:** applied
